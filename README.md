...
- **Pueden sacar a alguien de un torneo** desde "Quiénes se anotaron" en el detalle del torneo (por ejemplo, si no pagó): cada nombre tiene una × al lado, solo visible para el admin. Si esa persona ya tenía pareja armada, se borra la pareja y sus partidos pendientes junto con la inscripción; si ya jugó algún partido en el torneo, la app avisa y no lo borra solo — hay que sacarle antes la pareja o el resultado a mano, para no perder datos de partidos ya jugados.
