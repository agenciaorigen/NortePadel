-- Nueva función para "Tu próximo partido" (Inicio, solo con sesión iniciada):
-- el próximo partido SIN JUGAR de un jugador puntual, en cualquier torneo/categoría
-- en que esté participando. Hermana de mis_ultimos_partidos_publico (misma forma
-- de columnas, reutiliza la misma tarjeta llavePartidoCardHtml en app.js).
--
-- Corre esto una sola vez en el SQL Editor de Supabase. No toca funciones
-- existentes ni datos.

drop function if exists mi_proximo_partido_publico(uuid);

create or replace function mi_proximo_partido_publico(p_jugador_id uuid)
returns table (
  ...
  where pa.estado in ('programado', 'en_juego')
  order by pa.horario asc nulls last, pa.created_at asc
  limit 1;
$$;
